    ...
    <main className="min-h-screen bg-[#f8f9fa] pb-16 md:pb-24">
      ...
      <section className="relative bg-gradient-to-br from-[#0c1f38] via-[#0d2545] to-[#12365c] text-white pt-28 pb-16 md:py-32 px-4 sm:px-6 md:px-12 lg:px-24 overflow-hidden border-b border-[#2f55d4]/10">
        ...
        <div className="max-w-5xl mx-auto relative z-10 text-center">
          ...
          <motion.p 
            className="text-[#38bdf8] text-xs sm:text-sm md:text-base font-bold tracking-widest uppercase mb-6"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        {solutionsData.industries.map((ind, idx) => (
          ...
          >
            ...
            <div className={`lg:col-span-7 ${idx % 2 === 1 ? "lg:order-2" : "lg:order-1"} text-left`}>
              <span className="text-[10px] md:text-xs font-bold tracking-widest text-[#2f55d4] uppercase block mb-2 font-mono">
                {ind.subtitle}
              </span>
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-[#0d102c] mb-6">
                {ind.title}
              </h2>
              ...
              {/* YUNLU Solution */}
              <div className="mb-8">
                <h4 className="text-xs font-extrabold text-[#2f55d4] uppercase tracking-widest mb-3 flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#2f55d4]" />
                  云路三维编织解决方案
                </h4>
              ...
              <button
                ...
                className="px-6 py-2.5 bg-[#2f55d4] hover:bg-[#1d3c9f] text-white text-xs sm:text-sm font-bold rounded-lg shadow-sm cursor-pointer"
              >
                针对该行业咨询专属方案
              </button>
            </div>
                  ...
                  {ind.products.map((prod) => (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <span className="text-xs sm:text-sm font-bold text-white group-hover:text-[#38bdf8] transition-colors">
                          {prod.name}
                        </span>
        ...
        <section id="service-network" className="py-16 md:py-24 bg-white border-t border-neutral-200/50 scroll-mt-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 lg:px-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
              {/* Domestic Network */}
              <div className="p-6 sm:p-8 bg-[#f8f9fa] border border-neutral-200/60 rounded-3xl text-left">
                <div className="w-12 h-12 rounded-2xl bg-[#2f55d4]/10 text-[#2f55d4] flex items-center justify-center mb-6">
                ...
                <h3 className="text-xl font-bold text-[#0d102c] mb-2">{serviceData.domestic.title}</h3>
                <p className="text-neutral-500 text-sm sm:text-base font-light leading-relaxed mb-4">
                  {serviceData.domestic.description}
                </p>
                <div className="h-[2px] w-12 bg-[#2f55d4] rounded" />
              </div>
              ...
              <div className="p-6 sm:p-8 bg-[#f8f9fa] border border-neutral-200/60 rounded-3xl text-left">
                ...
                <h3 className="text-xl font-bold text-[#0d102c] mb-2">{serviceData.international.title}</h3>
                <p className="text-neutral-500 text-sm sm:text-base font-light leading-relaxed mb-4">
            ...
            <div className="p-6 sm:p-8 md:p-10 bg-gradient-to-br from-[#0c1f38] to-[#12365c] text-white rounded-3xl text-left relative overflow-hidden">
              <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/10 rounded-full blur-2xl pointer-events-none" />
              
              <h3 className="text-xl sm:text-2xl font-bold text-white mb-6 flex items-center gap-2">
                <span className="w-1.5 h-6 bg-[#2f55d4] rounded-full" />
              ...
              <ul className="space-y-4">
                {serviceData.cases.points.map((point, pIdx) => (
                  <li key={pIdx} className="flex items-start gap-4">
                    <div className="w-6 h-6 rounded-full bg-blue-500/20 text-[#38bdf8] flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold font-mono">
                      {pIdx + 1}
                    </div>
